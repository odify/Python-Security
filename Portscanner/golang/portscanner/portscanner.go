//portscanner  in golang (under construction)
...
func main() {
	...
	fmt.Print("[+] Enter the type of port for scanning e.g tcp: ")
	fmt.Scan(&typeOfPort)

	fmt.Print("[+] Scanning all open ports press : 1 \n[+]Scanning another ports press :2 \n")
	fmt.Scan(&option)
	if option == 1 {
